...
function validate(payload: BetaRequestPayload) {
  const fields = {
...
export async function POST(request: Request) {
  ...
  const to = process.env.BETA_REQUEST_TO_EMAIL;
  const from = process.env.BETA_REQUEST_FROM_EMAIL || "ReservKit Beta <onboarding@resend.dev>";

  if (!apiKey || !to) {
    return NextResponse.json(
      { error: "Beta request email is not configured." },
      { status: 500 }
    );
  }
  ...
  try {
    response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `ReservKit beta request: ${fields.businessName}`,
        text: lines.join("\n"),
      }),
    });
  } catch {
