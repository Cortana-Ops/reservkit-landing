...
Source commit `2f13d7b` (`Tighten Stripe payout and confirmation marketing copy`) is pushed to `main`. Verification passed full local `npm run verify`, `git diff --check`, and local served `RESERVKIT_MARKETING_BASE_URL=http://127.0.0.1:3012 npm run check:live` across 19 routes. Production is not updated yet: `vercel inspect https://reservkit.com --scope cortana-ops-projects --json` still reports Ready deployment `dpl_5wGGZFTRgvSjgJHTMAhKBerSN6tn`, and explicit `vercel deploy --prod --yes --scope cortana-ops-projects` failed with Vercel daily API quota `api-deployments-free-per-day`.
