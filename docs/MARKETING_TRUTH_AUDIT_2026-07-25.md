...
Source commit `0f107f9` (`Tighten public Stripe Connect shorthand`) is pushed to `main`. Verification passed full local `npm run verify`, `git diff --check`, local served `RESERVKIT_MARKETING_BASE_URL=http://127.0.0.1:3014 npm run check:live` across 19 routes, and focused rendered checks for `/`, `/roadmap`, and `/blog` across desktop and 390px mobile. Explicit Vercel production deploy failed with daily API quota `api-deployments-free-per-day`; `https://reservkit.com` still points at production deployment `dpl_9m7R6f4r8kZRxLHHdxgtjv6rjvtV`, and post-push production `check:live` correctly fails on the old homepage/Roadmap/Blog connected-account shorthand until Vercel catches up or the quota resets.
